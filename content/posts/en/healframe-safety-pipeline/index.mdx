...
faqs:
  ...
  - question: "How is the crisis-detection pipeline validated?"
    answer: "With an LLM-judge eval harness run against a zero-tolerance bar for missed crisis signals (false negatives). When classification is uncertain, the input stage fails closed — it treats the entry as a crisis."
---
...
Validation follows the same logic. I built an LLM-judge eval harness that repeatedly tests the pipeline against one non-negotiable bar: **zero missed crisis signals.** Every other metric is up for discussion. That one isn't.
