...
const fragmentShader = `
  ...
  void main() {
    ...
    float alpha = line * brightness * fadeY * 0.15;
...
export function LightTrails() {
  ...
  const uniforms = useMemo(
    () => ({
      uTime: { value: 0 },
      uColor1: { value: new THREE.Color('#c4a35a') },
  ...
  return (
    <group>
      ...
      <mesh ref={trailRef} position={[0, -totalHeight / 2 + 5, -3]}>
        <planeGeometry args={[0.5, totalHeight]} />
        <shaderMaterial
      ...
      <mesh position={[-6, -totalHeight / 2 + 5, -5]} rotation={[0, 0, 0.1]}>
        <planeGeometry args={[0.3, totalHeight * 0.8]} />
        <shaderMaterial
          vertexShader={vertexShader}
          fragmentShader={fragmentShader}
          uniforms={uniforms}
      ...
      <mesh position={[6, -totalHeight / 2 + 5, -4]} rotation={[0, 0, -0.08]}>
        <planeGeometry args={[0.2, totalHeight * 0.7]} />
        <shaderMaterial
          vertexShader={vertexShader}
          fragmentShader={fragmentShader}
          uniforms={uniforms}
